...
class GonghuiApi(BaseApi):
    ...
    # 公会踢人
    async def remove_members(self, remove_id: int):
        member_list = []
        temp = await self.client.callapi('clan/info', {'clan_id': 0, 'get_user_equip': 0})
        if 'clan' in temp:
        ...
        if self.viewer_id == leader_id and remove_id in member_list:
            await self.client.callapi('clan/remove', {'clan_id': self.clan_id, 'remove_viewer_id': remove_id})
            return '已移出' + str(remove_id) + '\n'

    # 公会放人
    async def accept_members(self, accept_list):
        temp = await self.client.callapi('clan/info', {'clan_id': 0, 'get_user_equip': 0})
        if 'clan' in temp:
            request = temp['have_join_request']
        if request:
            temp = await self.client.callapi('clan/join_request_list', {'clan_id': self.clan_id, 'page': 0, 'oldest_time': 0})
            for accept_id in accept_list:
                await self.client.callapi('clan/join_request_accept', {'request_viewer_id': accept_id, 'clan_id': self.clan_id})
        return True

    # 加入公会
    async def join_clan(self, clan_id: int):
        temp = await self.client.callapi('clan/join', {'clan_id': clan_id, 'from_invite': 0})
    ...
    # 解散工会
    async def break_clan(self):
        temp = await self.client.callapi('clan/breakup', {'clan_id': self.clan_id})
        if 'add_present_count' in temp:
            print('解散成功')

    # 退出工会
    async def leave_clan(self):
        temp = await self.client.callapi('clan/leave', {'clan_id': self.clan_id})
        if not temp:
            print('退出成功')

    # 交接会长
    async def change_role(self, change_id: int):
        temp = await self.client.callapi('clan/change_role', {'role_info': [
                                   {'viewer_id': self.viewer_id, 'role_id': 0}, {'viewer_id': change_id, 'role_id': 40}]})
        if 'members' in temp:
            print('交接成功')

    # 查询公会内成员详情
    async def query_clan_member(self, clan_id: int):
        # print('  行会id为'+str(clan_id))
        mem_list = []
        members = await self.client.callapi('clan/others_info', {'clan_id': clan_id})['clan']['members']
        for mem in members:
            res = await self.client.callapi(
